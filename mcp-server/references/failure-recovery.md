...
| `freecycle_check_availability` returns a cloud fallback payload | Ollama is down, FreeCycle is unreachable, or wake-and-wait timed out | Read the returned `message`, then check wake-on-LAN settings and host reachability before retrying |
...
| `freecycle_pull_model` returns `503` | Ollama is not running on the FreeCycle machine | Call `freecycle_check_availability`, then retry after local readiness succeeds |
